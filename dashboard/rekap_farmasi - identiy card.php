...
$role = strtolower(trim($medicRole));
        ...
        if (
            ...
            if ($medicName === '' || $medicJabatan === '') {
                ...
                if (empty($errors)) {
                    ...
                    $consumerId = !empty($_POST['consumer_id'])
                        ? (int)$_POST['consumer_id']
                        : null;

                    // ===============================
                    // TOTAL HARI INI (DB) — OPSI 3 (consumer_id based)
                    // ===============================

                    // 🔑 consumer_id WAJIB untuk transaksi baru
                    if (empty($consumerId)) {
                        $errors[] = "Konsumen wajib dipilih dari data yang terdaftar.";
                    } else {

                        // ===============================
                        // AMBIL TOTAL ITEM KONSUMEN HARI INI (BERDASARKAN consumer_id)
                        // ===============================
                        $stmt = $pdo->prepare("
                            SELECT 
                        ...
                        // ===============================
                        // VALIDASI 1 KONSUMEN = 1 TRANSAKSI / HARI (HARD RULE)
                        // ===============================
                        $totalToday =
                        ...
                        // ===============================
                        // HITUNG TOTAL SETELAH TRANSAKSI INI
                        // ===============================
                        $newBandage = (int)$totals['total_bandage'] + $addBandage;
                        $newIfaks   = (int)$totals['total_ifaks'] + $addIfaks;
                        $newPain    = (int)$totals['total_painkiller'] + $addPain;

                        // ===============================
                        // BATAS HARIAN (SOFT LIMIT)
                        // ===============================
                        $maxBandage = 30;
                        $maxIfaks   = 10;
                        $maxPain    = 10;
                        ...
                        if ($newBandage > $maxBandage) {
                            $warnings[] =
                                "⚠️ {$consumerName} melebihi batas BANDAGE ({$newBandage}/{$maxBandage}).";
                            $overLimit = true;
                        }

                        if ($newIfaks > $maxIfaks) {
                            $warnings[] =
                                "⚠️ {$consumerName} melebihi batas IFAKS ({$newIfaks}/{$maxIfaks}).";
                            $overLimit = true;
                        }

                        if ($newPain > $maxPain) {
                            $warnings[] =
                                "⚠️ {$consumerName} melebihi batas PAINKILLER ({$newPain}/{$maxPain}).";
                            $overLimit = true;
                        }

                        // ===============================
                        // BLOK JIKA MELEBIHI BATAS & TIDAK DI-OVERRIDE
                        // ===============================
                ...
                if (empty($errors)) {
                    ...
                    try {
                        ...
                        foreach ($selectedIds as $id) {
                            ...
                            // tx_hash unik per paket (anti double submit)
                            $txHash = hash('sha256', $postedToken . '|' . $id);

                            $stmtInsert->execute([
                                ':cid'     => $consumerId,     // 🔥 RELASI UTAMA OPSI 3
                                ':cname'   => $consumerName,   // legacy / display
                                ':mname'   => $medicName,
                                ':muid'    => $userId,
                                ':mjab'    => $medicJabatan,
                                ':pid'     => (int)$p['id'],
                        ...
                        if ($userId > 0) {
                            ...
                            // ======================================================
                            // 🔥 PASTIKAN SESSION FARMASI AKTIF
                            // ======================================================
            ...
            ");
                        ...
                        // ⛔ PASTIKAN TIDAK ADA DATA SETENGAH MASUK
                        if ($pdo->inTransaction()) {
                            $pdo->rollBack();
                        }

                        // ===============================
                        // ERROR HANDLING OPSI 3 (AKURAT)
                        // ===============================
                        if ($e->getCode() === '23000') {

                            $msg = 'Transaksi gagal karena data duplikat.';

                            // 🔒 KONSUMEN SUDAH TRANSAKSI HARI INI
                            if (stripos($e->getMessage(), 'uniq_consumer_per_day') !== false) {
                                $msg = '🚫 Konsumen ini sudah memiliki transaksi hari ini.';
                            }
                            // 🔁 DOUBLE SUBMIT / TX HASH
                            elseif (stripos($e->getMessage(), 'tx_hash') !== false) {
                                $msg = '⚠️ Transaksi ini sudah pernah diproses.';
                            }

                            $errors[] = $msg;
                        } else {
                            app_log('[INSERT ERROR] ' . $e->getMessage());
...
$sqlSales = "
    ...
    WHERE s.created_at >= :start
      AND s.created_at < DATE_ADD(:end, INTERVAL 1 SECOND)
";
...
<section class="content">
        ...
        <?php if ($medicName): ?>
            ...
            <div class="card">
                ...
                <form method="post" id="saleForm">
                    ...
                    <input type="hidden" name="consumer_id" id="consumerIdInput">
                    <div class="row-form-2">
                        ...
                        <div class="col">
                            ...
                            <div class="consumer-search-inline" style="position: relative;">

                                <!-- Input -->
                                <div class="consumer-input">
                                    <input
                                        type="text"
                                        name="consumer_name"
                                        id="consumerNameInput"
                                        autocomplete="off"
                                        placeholder="Ketik nama / Citizen ID"
                                        required>
                                </div>

                                <!-- ➕ Tombol Daftar Konsumen -->
                                <button
                                    type="button"
                                    class="btn btn-secondary btn-add-consumer"
                                    title="Daftar Konsumen Baru"
                                    onclick="openAddConsumerModal()">
                                    ➕
                                </button>

                                <!-- Dropdown hasil search consumers -->
                                <div
                                    id="consumerSearchDropdown"
                                    class="consumer-search-dropdown"
                                    style="display:none;">

                                </div>
                            </div>

                            <!-- Box warning / similar consumer -->
                            <div
                                id="similarConsumerBox"
                                class="consumer-similar-box"
                                style="display:none;">
                            </div>

                            <small>
                                Ketik nama, kalau sudah pernah beli akan muncul dan bisa diklik.
                                Mohon ketik nama sesuai KTP.
                            </small>
                        </div>
    ...
    <script>
        // 🔑 GLOBAL FLAG (DIPAKAI SEMUA SCRIPT)
        window.isProgrammaticSet = false;
    </script>

    <script>
        let nikCheckTimeout = null;
        let nikStatus = 'unknown'; // available | duplicate | conflict

        function checkNikRealtime() {
            const nik = document.getElementById('c_citizen_id').value.trim().toUpperCase();
            ...
            nikCheckTimeout = setTimeout(async () => {
                try {
                    ...
                    const res = await fetch('/ajax/check_nik.php', {
                        method: 'POST',
                        body: fd
                    });
        ...
        function handleNikResult(res) {
            const saveBtn = document.getElementById('btnSaveConsumer');

            if (res.status === 'available') {
                nikStatus = 'available';
                showToast('Citizen ID tersedia', 'success');
            ...
            if (res.status === 'conflict') {
                ...
                document.getElementById('conflictNik').textContent =
                    document.getElementById('c_citizen_id').value;

                openConflictModal(res.existing);
                return;
            }
        }
    </script>
    ...
    <script>
        function openCitizenConflictPopup(data) {
            const reason = prompt(
                `⚠️ KONFLIK DATA KONSUMEN

Citizen ID sama, tetapi nama berbeda.

Nama lama : ${data.oldName}
Nama baru : ${data.newName}

Masukkan alasan perubahan data:
- Salah nama
- Operasi plastik
- Masuk instansi
- Lainnya
`);
            ...
            if (!reason || reason.length < 5) {
                showToast('Alasan wajib diisi (min 5 karakter)', 'error');
                return;
            }

            updateCitizenIdentity(data, reason);
        }

        let CONFLICT_EXISTING = null;

        let CONFLICT_DATA = null;

        function openConflictModal(existing, newData) {
            CONFLICT_DATA = {
                existing: existing,
                new: newData
            };

            document.getElementById('conflictNik').textContent = newData.citizen_id;
        ...
        async function updateCitizenIdentity(data, reason) {
            const res = await fetch('/ajax/update_citizen_identity.php', {
                method: 'POST',
                headers: {
        ...
        async function submitConflictUpdate() {
            const ktpFile = document.getElementById('conflictKtpFile').files[0];
            const reason = document.getElementById('conflictReason').value;
            const note = document.getElementById('conflictNote').value;

            if (!ktpFile || !reason) {
                showToast('Foto KTP dan alasan wajib diisi', 'error');
                return;
            }

            const fd = new FormData();
            fd.append('nik', CONFLICT_DATA.new.citizen_id);
            fd.append('first_name', CONFLICT_DATA.new.first_name);
            fd.append('last_name', CONFLICT_DATA.new.last_name);
            fd.append('ktp_image', ktpFile);
            fd.append('reason', reason);
            fd.append('note', note);

            try {
                const res = await fetch('/ajax/update_consumer_conflict.php', {
                    method: 'POST',
                    body: fd
                });
                ...
                showToast('Data konsumen berhasil diperbarui', 'success');
                closeConflictModal();

                // isi kembali form transaksi
                document.getElementById('consumerIdInput').value = json.id;
                document.getElementById('consumerNameInput').value =
    ...
    <script>
        function disableSave(state) {
            const btn = document.getElementById('btnSaveConsumer');
            if (!btn) return;
            btn.disabled = state;
        }
    </script>
    ...
    <script>
        (function() {
            ...
            const dropdown = document.getElementById('consumerSearchDropdown');

            const nameInput = document.getElementById('consumerNameInput');
            const idInput = document.getElementById('consumerIdInput');

            nameInput.addEventListener('input', () => {
                if (isProgrammaticSet) return; // 🔑 JANGAN RESET
                idInput.value = '';
            });
            ...
            // FETCH DATA
            // =========================
            async function fetchConsumers(query) {
                try {
                    const res = await fetch(
                        '/ajax/search_consumers.php?q=' + encodeURIComponent(query), {
                            cache: 'no-store'
                        }
                    );
                    ...
                    console.error('Consumer search error:', err);
                    dropdown.style.display = 'none';
                }
            }
            ...
            // RENDER DROPDOWN
            // =========================
            function renderDropdown(consumers) {
                dropdown.innerHTML = consumers.map(c => {
                    const fullName = `${escapeHtml(c.first_name)} ${escapeHtml(c.last_name)}`;
                    const sex = (c.sex || '').toUpperCase();
                    const date = c.created_date || '';
                    const citizen = escapeHtml(c.citizen_id || '-');
                    const regBy = escapeHtml(c.registered_by || '-');

                    return `
                    <div class="consumer-search-item"
                        onclick="selectConsumerFromSearch(${c.id}, '${fullName.replace(/'/g, "\\'")}')">

                        <div class="consumer-search-name">${fullName}</div>
                        ...
                        <div class="consumer-search-registered">
                            Registered by ${regBy}
                        </div>
                    </div>
            `;
            ...
            // SELECT ITEM
            // =========================
            window.selectConsumerFromSearch = function(consumerId, fullName) {
                isProgrammaticSet = true;

                // 🔑 ISI INPUT SESUAI PILIHAN USER
                nameInput.value = fullName;
                idInput.value = consumerId; // ✅ INI YANG BENAR

                isProgrammaticSet = false;

                // ❌ JANGAN trigger input (karena akan reset ID)
    ...
    <script>
        ...
        // ⚠️ Catatan:
        // Validasi di JS hanya untuk tampilan.
        // Validasi FINAL ada di server (consumer_id).
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            // ✅ RESTORE OCR RESULT DARI LOCALSTORAGE
            const savedOCR = localStorage.getItem('ocr_result');
            if (savedOCR) {
                try {
                    const data = JSON.parse(savedOCR);

                    // Cek apakah data masih fresh (< 1 jam)
                    const now = Date.now();
                    const savedTime = localStorage.getItem('ocr_saved_time');
                    const oneHour = 60 * 60 * 1000;

                    if (!savedTime || (now - parseInt(savedTime)) < oneHour) {
                        // Data masih fresh
                        fillFormFromOCR(data);
                        showKtpPreview(data.temp_file);

                        showToast('Data OCR dipulihkan dari cache', 'info');
                    } else {
                        // Data kadaluarsa
                        localStorage.removeItem('ocr_result');
                        localStorage.removeItem('ocr_saved_time');
                    }
                } catch (e) {
                    console.error('Failed to restore OCR:', e);
                }
            }
            ...
            try {
                ...
                if (data.user_status === 'offline') {
                    ...
                    showFairnessNotice(`
                        ...
                        Silakan Klik Tombol <strong>OFFLINE</strong> utnuk merubah status menjadi <strong>ONLINE</strong> untuk melanjutkan.
                    `);
...
<!-- ===============================
     ...
     (STYLE SAMA DENGAN MODAL LAIN)
     =============================== -->
<div id="addConsumerModal" class="modal-overlay" style="display:none;">
    <div class="modal-box">

        <h3>➕ Daftar Konsumen Baru</h3>

        <!-- MODE SELECTOR -->
        <div class="mode-switch" style="display:flex;gap:8px;margin-bottom:14px;">
            <button type="button"
                class="btn-secondary active"
                id="btnModeManual"
                onclick="setConsumerMode('manual')">
                ✍️ Mode Manual
            </button>
        ...
        <!-- =========================
             MODE FOTO KTP
        ========================= -->
        <div id="modeKtp" style="display:none;">

            <div id="ktpPreviewBox" style="display:none;margin-top:12px;">
                <label>Preview KTP</label>
                <img id="ktpPreviewImg"
                    style="width:100%;max-width:220px;border-radius:10px;cursor:zoom-in;">
            </div>

            <!-- ✅ TOMBOL KOREKSI MANUAL -->
            <div id="ocrCorrectionBar" style="display:none;margin-top:10px;">
                <button type="button"
                    class="btn-secondary"
                    onclick="enableManualCorrection()"
                    style="width:100%;">
                    ✏️ Koreksi Data Manual
                </button>
                <small style="display:block;margin-top:6px;color:#64748b;font-size:12px;">
                    Klik jika ada data yang salah dibaca OCR
                </small>
            </div>

            <div id="imageZoomModal" style="display:none;" class="modal-overlay">
                <img id="imageZoomImg"
            ...
            <div class="row-form-1">
                <div class="col">
                    ...
                    <input type="file"
                        ...
                        capture="environment">
                    <small>
                        Ambil foto KTP dengan kamera atau upload file.
                    </small>
                </div>
            </div>
        ...
        <div id="ocrResultSection" style="display:none;">
            ...
            <div style="padding:12px;background:#dcfce7;border-radius:10px;margin-bottom:14px;border:1px solid #86efac;">
                <strong style="color:#166534;font-size:13px;">✅ Data berhasil dibaca dari KTP</strong>
                <p style="margin:4px 0 0;font-size:12px;color:#15803d;">
                    Silakan periksa data di bawah ini sebelum menyimpan.
                </p>
            </div>

            <!-- FORM DATA KONSUMEN -->
            <form class="form" onsubmit="event.preventDefault(); submitAddConsumer();">

                <div class="row-form-2">
                    <div class="col">
                        <label>First Name</label>
                        <input type="text" id="c_first_name" required readonly>
                    </div>

                    <div class="col">
                        <label>Last Name</label>
                        <input type="text" id="c_last_name" required readonly>
                    </div>
                </div>
                ...
                <div class="row-form-2">
                    <div class="col">
                        ...
                        <input type="date" id="c_dob" required readonly>
                    </div>

                    <div class="col">
                        <label>Jenis Kelamin</label>
                        <select id="c_sex" required disabled>
                ...
                <div class="row-form-2">
                    <div class="col">
                        ...
                        <input type="text" id="c_nationality" value="Indonesia" readonly>
                    </div>

                    <div class="col">
                        <label>Citizen ID (NIK)</label>
                        <input type="text" id="c_citizen_id" required readonly>
                    </div>
                </div>
                ...
                <div class="row-form-1">
                    <div class="col">
                        ...
                        <input type="text" id="c_pekerjaan" value="Freelance">
                    </div>
                </div>

                <!-- ✅ TOMBOL KOREKSI & SIMPAN -->
        ...
        <!-- =========================
             FORM DATA KONSUMEN
             (dipakai oleh KEDUA MODE)
        ========================= -->
        <div id="modeManual" style="display:none;visibility:hidden;height:0;overflow:hidden;">
            <form class="form" onsubmit="event.preventDefault(); submitAddConsumer();">

                <div class="row-form-2">
                    <div class="col">
                        <label>First Name</label>
                        <input type="text" id="c_first_name" required>
                    </div>

                    <div class="col">
                        <label>Last Name</label>
                        <input type="text" id="c_last_name" required>
                    </div>
                </div>
                ...
                <div class="row-form-2">
                    <div class="col">
                        <label>Nationality</label>
                        <input type="text" id="c_nationality" value="Indonesia">
                    </div>
                ...
                <div class="modal-actions">
                    <button type="button"
                        class="btn-secondary"
                        onclick="closeAddConsumerModal()">
                        Batal
                    </button>

                    <button id="btnSaveConsumer" class="btn-success" disabled>
                        Simpan
                    </button>
                </div>
...
<div id="conflictModal" class="modal-overlay" style="display:none;">
    <div class="modal-box">
        ...
        <h3>⚠️ Konflik Citizen ID</h3>

        <p style="font-size:14px;">
            Citizen ID <strong id="conflictNik"></strong> sudah terdaftar
            dengan nama berbeda.
        </p>

        <p style="font-size:13px;color:#6b7280;">
            Data lama akan <strong>digantikan</strong>.
            Mohon upload KTP terbaru dan isi alasan perubahan data.
        </p>

        <div class="row-form-1">
            <label>Foto KTP Baru</label>
            <input type="file" id="conflictKtpFile" accept="image/*" required>
        </div>

        <div class="row-form-1">
            <label>Alasan Perubahan Data</label>
            <select id="conflictReason" required>
                <option value="">-- Pilih alasan --</option>
                <option value="Salah input nama sebelumnya">Salah input nama</option>
                <option value="Ganti nama resmi">Ganti nama resmi</option>
                <option value="Operasi plastik">Operasi plastik</option>
                <option value="Masuk instansi wajib ganti nama">Masuk instansi (wajib ganti nama)</option>
        ...
        <div class="modal-actions">
            <button class="btn-secondary" onclick="closeConflictModal()">Batal</button>
            <button class="btn-danger" onclick="submitConflictUpdate()">
                Update Data
            </button>
        </div>
...
<script>
    ...
    function isConsumerFormValid() {
        const first = document.getElementById('c_first_name')?.value.trim();
        const last = document.getElementById('c_last_name')?.value.trim();
        const nik = document.getElementById('c_citizen_id').value.trim().toUpperCase();
        // Wajib diisi
        if (!nik) {
            showToast('Citizen ID wajib diisi', 'error');
            document.getElementById('c_citizen_id').focus();
            return;
        }

        // Format Citizen ID FiveM (huruf besar + angka)
        if (!/^[A-Z0-9]{6,20}$/.test(nik)) {
            showToast(
                'Citizen ID harus huruf BESAR dan angka (6–20 karakter, tanpa spasi)',
                'error'
            );
            document.getElementById('c_citizen_id').focus();
            return;
        ...
        // pastikan nilai disimpan uppercase
        document.getElementById('c_citizen_id').value = nik;
        const dob = document.getElementById('c_dob')?.value;
        const sex = document.getElementById('c_sex')?.value;

        return !!(first && last && nik && dob && sex);
    }
    ...
    function updateSaveButtonState() {
        ...
        // belum pilih mode → disable
        if (!CURRENT_CONSUMER_MODE) {
            btn.disabled = true;
            return;
        }

        // mode manual → validasi form
        if (CURRENT_CONSUMER_MODE === 'manual') {
            btn.disabled = !isConsumerFormValid();
            return;
        }

        // mode ktp → tunggu OCR (akan diaktifkan manual)
        if (CURRENT_CONSUMER_MODE === 'ktp') {
            // enable hanya kalau field hasil OCR sudah terisi
            btn.disabled = !isConsumerFormValid();
        }
    }
</script>
...
<script>
    document.addEventListener('input', function(e) {
        if (e.target.closest('#modeManual')) {
            updateSaveButtonState();
        }
    });
...
<script>
    let CURRENT_CONSUMER_MODE = null; // belum pilih apa-apa

    function setConsumerMode(mode) {
        const btnManual = document.getElementById('btnModeManual');
        const btnKtp = document.getElementById('btnModeKtp');
        const modeKtp = document.getElementById('modeKtp');
        const modeManual = document.getElementById('modeManual');

        if (!btnManual || !btnKtp || !modeKtp || !modeManual) return;

        CURRENT_CONSUMER_MODE = mode;

        // reset semua tampilan
        modeKtp.style.display = 'none';
        modeManual.style.display = 'none';

        btnManual.classList.remove('active');
        btnKtp.classList.remove('active');
        ...
        if (mode === 'ktp') {
            modeKtp.style.display = 'block';
            btnKtp.classList.add('active');

            // ✅ CEK APAKAH ADA DATA OCR TERSIMPAN
            const savedOCR = localStorage.getItem('ocr_result');
            if (savedOCR) {
                try {
                    const data = JSON.parse(savedOCR);
    ...
    // MODAL DIBUKA → SEMUA DISEMBUNYIKAN
    function openAddConsumerModal() {
        ...
        // reset state
        CURRENT_CONSUMER_MODE = null;

        if (modeKtp) modeKtp.style.display = 'none';
...
<script>
    function openAddConsumerModal() {
        document.getElementById('addConsumerModal').style.display = 'flex';
        document.body.classList.add('modal-open');
    }

    function closeAddConsumerModal() {
        document.getElementById('addConsumerModal').style.display = 'none';
        document.body.classList.remove('modal-open');
        // ✅ RESET TAMPILAN
        document.getElementById('ocrResultSection').style.display = 'none';
        document.getElementById('modeKtp').style.display = 'none';
        document.getElementById('modeManual').style.display = 'none';

        // Reset button "Ambil Data"
        const ambilBtn = document.querySelector('#modeKtp button');
        if (ambilBtn) ambilBtn.style.display = 'block';
        ...
        // Reset preview KTP
        document.getElementById('ktpPreviewBox').style.display = 'none';
    }

    async function submitAddConsumer() {
        const payload = {
            ...
            citizen_id: document.getElementById('c_citizen_id').value.trim(),
            pekerjaan: document.getElementById('c_pekerjaan').value.trim(),
        };

        if (nikStatus === 'duplicate') {
            showToast('Citizen ID sudah terdaftar, tidak bisa disimpan', 'error');
            return;
        }

        if (!payload.first_name || !payload.last_name || !payload.dob || !payload.citizen_id) {
            alert('❌ Semua field wajib diisi.');
            return;
        }

        try {
            const res = await fetch('/ajax/store_consumer.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            const json = await res.json(); // 🔥 INI YANG KURANG

            if (!json.success) {
                alert(json.message || 'Gagal menyimpan konsumen');
                return;
            }
            ...
            nameInput.value = payload.first_name + ' ' + payload.last_name;
            ...
            saveFormState();
            recalcTotals();
            // ✅ HAPUS CACHE OCR SETELAH BERHASIL
            localStorage.removeItem('ocr_result');
            localStorage.removeItem('ocr_saved_time');
            closeAddConsumerModal();
            alert('✅ Konsumen berhasil didaftarkan dan siap dipakai');
...
<script>
    async function processKtpImage(btn) {
        console.log('OCR CLICKED');
        // ✅ AUTO-DETECT NIK DARI NAMA FILE (OPSIONAL)
        document.getElementById('ktp_image').addEventListener('change', function(e) {
            const fileName = e.target.files[0]?.name || '';

            // Cek apakah nama file mengandung pattern Citizen ID
            // Contoh: KTP_ABC12345.jpg
            const match = fileName.match(/[A-Z]{2,4}\d{6,10}/i);

            if (match) {
                showToast(`📄 Citizen ID terdeteksi dari file: ${match[0]}`, 'info');
            }
        });
        const fileInput = document.getElementById('ktp_image');
        if (!fileInput || !fileInput.files.length) {
            showToast('Pilih foto KTP terlebih dahulu', 'error');
            return;
        }

        btn.disabled = true;
        const originalText = btn.textContent;
        btn.textContent = '⏳ Memproses OCR...';

        const formData = new FormData();
        formData.append('image', fileInput.files[0]); // 🔥 SESUAI BACKEND
        formData.append('action', 'ocr_ajax'); // 🔥 WAJIB

        try {
            const res = await fetch('/ajax/ocr_ktp.php', {
                method: 'POST',
                body: formData
            });
            ...
            // Cek conflict
            if (json.conflict) {
                const oldName = `${json.existing.first_name} ${json.existing.last_name}`;
                const newName = `${json.first_name} ${json.last_name}`;

                const confirmMsg =
                    `⚠️ KONFLIK DATA DITEMUKAN\n\n` +
                    `Citizen ID: ${json.citizen_id}\n` +
                    `Nama Lama: ${oldName}\n` +
                    `Nama Baru: ${newName}\n\n` +
                    `Data ini sudah terdaftar dengan nama berbeda.\n` +
                ...
                // Buka conflict modal
                openConflictModal(json.existing, json);
                return;
            }

            // Cek duplicate
            if (json.duplicate) {
                showToast('Citizen ID sudah terdaftar dengan nama yang sama', 'error');

                // Isi form langsung
                document.getElementById('consumerIdInput').value = json.existing_id;
                document.getElementById('consumerNameInput').value =
                    `${json.first_name} ${json.last_name}`;
            ...
            localStorage.setItem('ocr_result', JSON.stringify(json));
            localStorage.setItem('ocr_saved_time', Date.now().toString());
            ...
            // ✅ ISI FORM OTOMATIS
            fillFormFromOCR(json);

            // ✅ TAMPILKAN PREVIEW KTP
            showKtpPreview(json.temp_file);

            // ✅ ENABLE TOMBOL SIMPAN
            const saveBtn = document.getElementById('btnSaveConsumer');
            if (saveBtn) saveBtn.disabled = false;

            showToast('OCR berhasil dibaca, silakan periksa data', 'success');

        } catch (e) {
            console.error('FETCH ERROR:', e);
            showToast('Gagal menghubungi server OCR', 'error');
        } finally {
            btn.disabled = false;
...
<script>
    ...
    function fillFormFromOCR(data) {
        // ✅ ISI FORM
        document.getElementById('c_first_name').value = data.first_name || '';
        document.getElementById('c_last_name').value = data.last_name || '';
        document.getElementById('c_dob').value = data.dob || '';

        const sexValue = (data.sex || '').toLowerCase();
        document.getElementById('c_sex').value = sexValue;

        document.getElementById('c_nationality').value = data.nationality || 'Indonesia';
        document.getElementById('c_citizen_id').value = data.citizen_id || '';

        // ✅ JIKA NIK KOSONG
        const nikInput = document.getElementById('c_citizen_id');
        if (!data.citizen_id) {
            nikInput.removeAttribute('readonly');
            nikInput.focus();
            showToast('NIK tidak terbaca, silakan isi manual', 'warning');
        } else {
            nikInput.setAttribute('readonly', true);
        }
        // ✅ ENABLE SAVE BUTTON (WALAUPUN NIK KOSONG)
        const saveBtn = document.getElementById('btnSaveConsumer');
        if (saveBtn) {
            saveBtn.disabled = false; // ⬅️ INI KUNCI
        }

        // ✅ TAMPILKAN SECTION HASIL OCR
        const resultSection = document.getElementById('ocrResultSection');
        if (resultSection) {
            resultSection.style.display = 'block';
        }

        // ✅ SEMBUNYIKAN TOMBOL "AMBIL DATA"
        const modeKtp = document.getElementById('modeKtp');
        if (modeKtp) {
            const ambilBtn = modeKtp.querySelector('button');
            if (ambilBtn) ambilBtn.style.display = 'none';
        }
    ...
    function enableManualCorrection() {
        ...
        showToast('📝 Mode koreksi aktif - edit data yang salah', 'info');
    ...
    // ✅ TAMPILKAN PREVIEW KTP
    function showKtpPreview(tempFile) {
        if (!tempFile) return;

        const img = document.getElementById('ktpPreviewImg');
        const box = document.getElementById('ktpPreviewBox');

        // Convert path: hapus prefix berlebih
        const cleanPath = tempFile.replace(/.*farmasi-ems/, '');
        img.src = cleanPath;
        box.style.display = 'block';

        // Klik untuk zoom
        img.onclick = () => openImageZoom(img.src);
        // ✅ JIKA ADA DATA YANG KOSONG, TAMPILKAN WARNING
        const nikInput = document.getElementById('c_citizen_id');
        ...
        ⚠️ <strong>Citizen ID tidak terbaca</strong><br>
                Silakan isi manual menggunakan tombol <strong>Koreksi Manual</strong>
            `;

            box.parentElement.appendChild(warningBox);
        }
    }
</script>
